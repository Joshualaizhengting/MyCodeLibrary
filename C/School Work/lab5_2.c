...
/* Write your program code here */
    ...
    /* for number of staff records -> into the struc leaverecord each element is assigned */
    for (int j = 0; j<*n; j++){
        printf("Enter id, totalleave, leavetaken: \n");
        scanf("%d %d %d", &list[j].id, &list[j].totalLeave, &list[j].leaveTaken);
    }
}
...
/* Write your program code here */
    ...
    /* iterate throught the whole struc list to find the id where it occurs, find the remainder between the leaves return based on how much leave remains*/
    for (int i = 0; i<n; i++){
        if (list[i].id == id){
            remainder = list[i].totalLeave - list[i].leaveTaken;
            if (remainder > leave){
                return 1;
            }else{
                return 0;
