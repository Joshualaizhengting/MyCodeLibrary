...
class Trie:
    ...
    def find_words_with_prefix(self, prefix): #question 2
        ...
        self.collect_all(node, prefix, results)
        return results
    ...
    def find_shortest_word_with_prefix(self, prefix):#question 3
            ...
            def bfs(start, prefix):
                queue = Queue()
                queue.enqueue((start, prefix))

                while not queue.is_empty():
                    node, curr = queue.dequeue()

                    if node.is_end_of_word:
                        shortest_words.append(curr)
                        return 
                
                    child = node.first_child

                    while child:
                        queue.enqueue((child, curr+child.char))
                        child = child.next_sibling
            
            bfs(node, prefix)
            return shortest_words
            
    def collect_all(self, node, prefix, results):
        if node.is_end_of_word:
            results.append(prefix)
        child = node.first_child
        while child:
            self.collect_all(child, prefix+child.char, results)
            child = child.next_sibling
